...
export class TablaComponent implements OnInit {
  ...
  nuevo(){
    this.router.navigate([`/medium/home/proyectos/${this.id_proyecto}/gmao/configuracion/tipo-mantenimiento-preventivo/nuevo`]);
  }
  ver(item){
   localStorage.setItem('tipo_incidencias_ver',JSON.stringify(item));
   this.router.navigate([`/medium/home/proyectos/${this.id_proyecto}/gmao/configuracion/tipo-mantenimiento-preventivo/ver`]);
  }
  buscar(item){
    console.log(item);
    if (item==''){
      ...
      const result = this.data.filter(x=>x.descripcion.toLowerCase().indexOf(item)> -1 );
      if (result.length>0){
        this.data=result;
      console.log(result);
      }else{ 
