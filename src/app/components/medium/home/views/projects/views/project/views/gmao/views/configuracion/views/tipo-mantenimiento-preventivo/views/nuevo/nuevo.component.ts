...
export class NuevoComponent implements OnInit {
  ...
  guardar(){
    if (this.datos.descripcion==''){
      ...
      this.datos.id_empresa=this.id_proyecto;
       this.gmao.add_tipo_mantenimiento_preventivo(this.datos).subscribe(
        variable_instanciada=>{
          let resultado=variable_instanciada;
          if(resultado.status==true){
            Swal.fire({
              icon: 'success',
              title: 'Tipo mantenimiento preventivo guardado',
              showConfirmButton: false,
              timer: 1500
                // según el plugin que nos mande el backend, entraremos en una u otra plataforma
              });
            this.router.navigate([`/medium/home/proyectos/${this.id_proyecto}/gmao/configuracion/tipo-mantenimiento-preventivo`]);
          }
        },
        err =>{
            if(err.url==null){
